...
import { useGoldTradeCount, useOwnerCount, useGoldInventoryCount } from 'lib/roq';

import { GoldInventoryListPage as ShowCaseList } from 'pages/gold-inventories';

function DashboardPage() {
  const { session } = useSession();

  const goldTradeStat = useGoldTradeCount();

  const ownerStat = useOwnerCount();

  const goldInventoryStat = useGoldInventoryCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Gold Trades',
      result: goldTradeStat?.data,
      isLoading: goldTradeStat.isLoading,
    ...
    },
    {
      ...
      result: goldInventoryStat?.data,
      isLoading: goldInventoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Gold Inventories
                </Text>
                <NextLink href={`gold-inventories`} passHref legacyBehavior>
                  <Button
