...
import { useRoqClient, useGoldTradeFindFirst } from 'lib/roq';
...
function GoldTradeEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGoldTradeFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GoldTradeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.gold_trade.update({
        data: values as any,
        where: {
          id,
        },
