...
import { useRoqClient, useGoldTradeFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<GoldTradeInterface, unknown>;

interface GoldTradeListPageProps {
...
export function GoldTradeListPage(props: GoldTradeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useGoldTradeFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.gold_trade.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: GoldTradeInterface) => {
    if (hasAccess('gold_trade', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/gold-trades/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.trade_date ? format(record?.trade_date, 'dd-MM-yyyy') : '',
    },
    { id: 'trade_price', header: 'trade_price', accessorKey: 'trade_price' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/gold-trades/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('gold_trade', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/gold-trades/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('gold_trade', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="gold_trade" />
        </Flex>

        {hasAccess('gold_trade', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/gold-trades/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
